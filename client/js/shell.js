...
export async function initShell({ active, title }) {
  ...
  aside.innerHTML = `
    <a href="${user.role === 'admin' ? '/pages/admin/dashboard.html' : '/pages/dashboard.html'}" class="flex items-center gap-2.5 px-6 h-16 border-b border-slate-200 dark:border-slate-800">
    ...
    <div class="px-3 py-4 border-t border-slate-200 dark:border-slate-800 space-y-1">
      ${user.role === 'admin' ? `
      <a href="/pages/admin/dashboard.html" class="flex w-full items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium
        text-brand-600 dark:text-brand-400 hover:bg-brand-50 dark:hover:bg-brand-900/30 transition-colors">
