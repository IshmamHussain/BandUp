// Listening test: timer, question navigation, submission, results review.
...
if (!testId) window.location.href = '/pages/listening.html';

// ---------- Load test ----------
let test;
try {
  test = await api.listeningTest(testId);
} catch (err) {
  toast(err.message, 'error');
  setTimeout(() => (window.location.href = '/pages/listening.html'), 1200);
  throw err;
}
...
paragraphs.forEach((text) => {
  ...
  p.textContent = text;
  el('transcript-body').appendChild(p);
});
...
// ---------- Timer (counts down from the test time limit) ----------
...
el('submit-btn').addEventListener('click', () => {
  const unanswered = test.questions.length - answers.size;
  if (unanswered > 0) {
    el('confirm-text').textContent =
      `${unanswered} of ${test.questions.length} questions ${unanswered === 1 ? 'is' : 'are'} still blank. Unanswered questions count as incorrect.`;
...
async function submit() {
  ...
  try {
    const result = await api.submitListening(testId, payload);
...
function renderResults(result) {
  ...
  transcriptBody.classList.remove('blur-sm', 'opacity-50', 'select-none');
