...
async function seed() {
  // 1. Delete the old test and its questions
  await pool.execute('DELETE FROM listening_tests WHERE id = 3 OR title = "Listening Test 1"');

  // 2. Insert Test 2
  const [res2] = await pool.execute(
    'INSERT INTO listening_tests (title, audio_url, transcript, difficulty, time_limit) VALUES (?, ?, ?, ?, ?)',
    ['Listening Test 2', 'https://ielts-up.com/listening/14.2.mp3', 'Transcript for Test 2...', 'medium', 30]
  );
  ...
  const [res3] = await pool.execute(
    'INSERT INTO listening_tests (title, audio_url, transcript, difficulty, time_limit) VALUES (?, ?, ?, ?, ?)',
    ['Listening Test 3', 'https://ielts-up.com/listening/14.3.mp3', 'Transcript for Test 3...', 'medium', 30]
  );
